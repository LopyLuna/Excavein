...
public class InteractionUtils {
    ...
    public InteractionResult useItemOn(ServerPlayer pPlayer, Level pLevel, ItemStack pStack, InteractionHand pHand, BlockHitResult pHitResult) {
        ...
        if (this.gameModeForPlayer != GameType.SPECTATOR) {
            ...
            if ((event.getUseItem() != Event.Result.DENY) && isProperItem(pStack.getItem())) {
            ...
            if (event.getUseBlock() == Event.Result.ALLOW || (event.getUseBlock() != Event.Result.DENY && !flag1) && HAND_INTERACTION.get()) {
            ...
            if ((event.getUseItem() == Event.Result.ALLOW || (!pStack.isEmpty() && !pPlayer.getCooldowns().isOnCooldown(pStack.getItem()))) && isProperItem(pStack.getItem())) {
                if (event.getUseItem() == Event.Result.DENY) return InteractionResult.PASS;
                InteractionResult interactionresult;
                if (pPlayer.isCreative()) {
                    int i = pStack.getCount();
                    interactionresult = pStack.useOn(useoncontext);
                    pStack.setCount(i);
                } else interactionresult = pStack.useOn(useoncontext);
                if (interactionresult.consumesAction()) CriteriaTriggers.ITEM_USED_ON_BLOCK.trigger(pPlayer, blockpos, itemstack);
                return interactionresult;
            }
        }
