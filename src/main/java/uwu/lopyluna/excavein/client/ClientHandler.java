...
public class ClientHandler {
    ...
    public static void onClientTick(ClientTickEvent event) {
        if (mc.getConnection() != null && event.phase.equals(TickEvent.Phase.END) && !Minecraft.getInstance().isPaused() && mc.player != null) {
    ...
    public static void onKey(InputEvent.Key event) {
        if (mc.getConnection() != null && !Minecraft.getInstance().isPaused() && mc.level != null && uuid != null && isNotFakePlayer(mc.level.getPlayerByUUID(uuid))) {
            ...
            Excavein.CHANNEL.sendToServer(new ModesPacket(uuid, keyPressed, switchMode));

            if (!shapeKeys.isEmpty())
                ExcaveinEntries.getShapeEntries().forEach((integer, shapeEntry) -> {
                    if (shapeEntry.hasKeybind()) Excavein.CHANNEL.sendToServer(new KeybindPacket(uuid, shapeKeys.get(shapeEntry).consumeClick(), integer, "shape"));
                });
            if (!modifierKeys.isEmpty())
                ExcaveinEntries.getShapeModifierEntries().forEach((integer, modifierEntry) -> {
                    if (modifierEntry.hasKeybind()) Excavein.CHANNEL.sendToServer(new KeybindPacket(uuid, modifierKeys.get(modifierEntry).consumeClick(), integer, "modifier"));
                });
        }
    }
    ...
    public static void onMouseScroll(InputEvent.MouseScrollingEvent event) {
        if (mc.getConnection() != null && !Minecraft.getInstance().isPaused() && mc.level != null && uuid != null && isNotFakePlayer(mc.level.getPlayerByUUID(uuid))) {
            if (keyPressed && !DISABLE_SCROLL.get()) {
                int switchMode = 0;
                if (MODIFIER_SCROLL.isDown() && event.getScrollDelta() > 0) switchMode = 3;
                else if (MODIFIER_SCROLL.isDown() && event.getScrollDelta() < 0) switchMode = 4;
                else if (event.getScrollDelta() > 0) switchMode = 1;
                else if (event.getScrollDelta() < 0) switchMode = 2;

                Excavein.CHANNEL.sendToServer(new ModesPacket(uuid, true, switchMode));
